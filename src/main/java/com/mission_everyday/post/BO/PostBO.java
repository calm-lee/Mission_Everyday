...
public class PostBO {
	...
	// 글쓰기
	public int addPost(int userId, String userName, int missionId, String missionName, String content,
			MultipartFile file, Date missionStartDate, Date missionFinishDate, int missionPeriod) {
		...
		return postDAO.insertPost(userId, userName, missionId, missionName, content, imgPath, missionStartDate,
				missionFinishDate, missionPeriod);
	}

	// 글 수정
	public int updatePost(int id, int userId, String content, MultipartFile file) {

		if (file != null) { // file도 수정하는 경우
			String imgPath = null;
			try {
				imgPath = fileManagerService.saveFile(userId, file);
			} catch (IOException e) {
		...
		} else { // file 수정 안 하는 경우
			Post post = postDAO.selectPostByUserIdAndPostId(userId, id);
			String imgPath = post.getImgPath(); // 기존 imgPath 가져옴
			return postDAO.updatePost(id, userId, content, imgPath);
		}
